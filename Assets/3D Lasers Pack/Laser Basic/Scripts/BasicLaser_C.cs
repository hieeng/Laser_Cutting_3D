...
public class BasicLaser_C : MonoBehaviour {
    ...
    void Start()
    {
        ...
        if(cam){
            ...
            CamPos = StartPoint.transform.position;
        }
    ...
    void LateUpdate()
    {
        ...
        if (LaserOn)
        {
            float CamDistSource = Vector3.Distance(StartPoint.position, CamPos);
            float CamDistEnd = Vector3.Distance(EndPos, CamPos);
            ViewAngle = Vector3.Angle(LaserDir, CamPos*1);
                ...
                if (hit2D)
                {
                    ...
                    if (EndFlare)
                    {
                        ...
                        if (EndFlareOffset > 0)
                            ...
                            EndFlare.transform.position = Vector3.Lerp(hit2D.point, StartPoint.position, EndFlareOffset/2);
                        else
            ...
            ///Else 3D Ray
                ...
                if (Physics.Raycast(ray, out hit, LaserDist, LaserMask)){
                    ...
                    EndPos = hit.point;
                    //EndPos = hit.point - StartPoint.localPosition;
                    ...
                    if (EndFlare)
                    {
                        ...
                        if (EndFlareOffset > 0)
                            ...
                            EndFlare.transform.position = Vector3.Lerp(hit.point, StartPoint.position, EndFlareOffset/2);
                        else
                            EndFlare.transform.position = EndPos;
                    }
                }
